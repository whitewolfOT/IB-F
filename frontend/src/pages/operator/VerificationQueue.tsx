...
const VerificationQueue: React.FC = () => {
  const { data: events, isLoading, error } = useEvents({ state: ApprovalState.under_review });
      ...
      {isLoading ? (
        ...
        <Card>
          {!events || events.length === 0 ? (
            <EmptyState title="Queue empty" message="No events awaiting operational verification." />
          ) : (
              ...
              <tbody>
                {events.map((ev) => (
                  <tr key={ev.event_id} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="py-2 font-mono">{ev.amount ?? '—'}</td>
                    ...
                    <td className="py-2 text-right">
                      <Link to={`/verify/${ev.event_id}`} className="text-operator text-xs hover:underline">Verify →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
