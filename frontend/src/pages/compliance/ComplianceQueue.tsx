...
const ComplianceQueue: React.FC = () => {
  const { data: events, isLoading, error } = useEvents({ state: ApprovalState.compliance_review });

  return (
    <div className="flex flex-col gap-6">
      <h2 className="text-xl font-bold text-gray-900">Compliance Queue</h2>
      ...
      {isLoading ? (
        ...
        <Card>
          {!events || events.length === 0 ? (
            <EmptyState title="Queue empty" message="No events awaiting compliance review." />
          ) : (
              ...
              <tbody>
                {events.map((ev) => (
                  <tr key={ev.event_id} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="py-2">
                      {typeof ev.compliance_score === 'number' ? (
                        <ScoreRing score={ev.compliance_score} size={48} />
                      ) : '—'}
                    </td>
                    <td className="py-2"><Badge state={ev.state} /></td>
                    <td className="py-2 text-right">
                      <Link to={`/compliance/${ev.event_id}`} className="text-comply text-xs hover:underline">Review →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
