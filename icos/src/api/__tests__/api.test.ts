...
describe('GET /api/contracts', () => {
  it('lists all contracts', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'c1', contract_type: 'murabaha' });
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'c2', contract_type: 'salam' });
    const res = await request(app).get('/api/contracts').set('Authorization', `Bearer ${masterToken()}`);
    expect(res.status).toBe(200);
  ...
  it('filters contracts by status', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'c1', contract_type: 'murabaha' });
    const res = await request(app).get('/api/contracts?status=draft').set('Authorization', `Bearer ${masterToken()}`);
    expect(res.status).toBe(200);
...
describe('GET /api/contracts/:id', () => {
  it('returns contract with empty ledger and audit trail', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-001', contract_type: 'murabaha' });
    const res = await request(app).get('/api/contracts/ctr-001').set('Authorization', `Bearer ${masterToken()}`);
    expect(res.status).toBe(200);
  ...
  it('returns 404 for unknown contract', async () => {
    const { app } = makeApp();
    const res = await request(app).get('/api/contracts/non-existent').set('Authorization', `Bearer ${masterToken()}`);
...
describe('POST /api/events/:id/transition', () => {
  ...
  it('transitions event from draft to submitted and returns audit event', async () => {
    ...
    const res = await request(app)
      ...
      .set('Authorization', `Bearer ${makeToken(OrgRole.operator, 'operator-001')}`)
      .send({
        newState: 'submitted',
  ...
  it('returns 422 for invalid state transition', async () => {
    ...
    const res = await request(app)
      .post(`/api/events/${eventId}/transition`)
      .set('Authorization', `Bearer ${makeToken(OrgRole.operator, 'op-001')}`)
      .send({
        newState: 'settled',
  ...
  it('returns 404 for unknown event', async () => {
    const { app } = makeApp();
    const res = await request(app)
      .post('/api/events/no-such-id/transition')
      .set('Authorization', `Bearer ${makeToken(OrgRole.operator, 'x')}`)
      .send({
        newState: 'submitted',
        reason: 'test',
      });
...
describe('POST /api/events/:id/pipeline', () => {
  it('runs murabaha pipeline and persists ledger entries', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-m-001', contract_type: 'murabaha' });
    ...
    const steps: [string, OrgRole][] = [
      ['submitted', OrgRole.operator],
      ['under_review', OrgRole.operator],
      ['operationally_verified', OrgRole.warehouse_manager],
      ['financially_verified', OrgRole.financial_controller],
      ['compliance_review', OrgRole.compliance_officer],
      ['approved', OrgRole.compliance_officer],
    ];
    for (const [state, role] of steps) {
      await request(app)
        .post(`/api/events/${eventId}/transition`)
        .set('Authorization', `Bearer ${makeToken(role, 'reviewer-001')}`)
        .send({
          newState: state,
    ...
    const contractRes = await request(app).get('/api/contracts/ctr-m-001').set('Authorization', `Bearer ${masterToken()}`);
...
describe('PATCH /api/reviews/:id/ruling', () => {
  it('applies a compliant ruling to a persisted review stub', async () => {
    ...
    const res = await request(app)
      .patch('/api/reviews/rev-001/ruling')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
  ...
  it('applies a non_compliant ruling and sets freeze flags', async () => {
    ...
    const res = await request(app)
      .patch('/api/reviews/rev-002/ruling')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
  ...
  it('returns 404 for unknown review', async () => {
    const { app } = makeApp();
    const res = await request(app)
      .patch('/api/reviews/no-such-review/ruling')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
        ruling_type: 'compliant',
...
describe('GET /api/parties', () => {
  it('returns empty list when no parties registered', async () => {
    const { app } = makeApp();
    const res = await request(app).get('/api/parties').set('Authorization', `Bearer ${masterToken()}`);
    expect(res.status).toBe(200);
...
describe('GET /api/assets', () => {
  it('returns empty list when no assets registered', async () => {
    const { app } = makeApp();
    const res = await request(app).get('/api/assets').set('Authorization', `Bearer ${masterToken()}`);
    expect(res.status).toBe(200);
    expect(res.body).toEqual([]);
  });
...
describe('GET /api/events', () => {
  it('returns empty list when no events created', async () => {
    const { app } = makeApp();
    const res = await request(app).get('/api/events').set('Authorization', `Bearer ${masterToken()}`);
    expect(res.status).toBe(200);
    expect(res.body).toEqual([]);
  });
  ...
  it('lists events and filters by contract_id', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-list-001', contract_type: 'murabaha' });
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-list-002', contract_type: 'salam' });
    await request(app)
      .post('/api/events')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
    ...
    const allRes = await request(app).get('/api/events').set('Authorization', `Bearer ${masterToken()}`);
    expect(allRes.status).toBe(200);
    expect(allRes.body).toHaveLength(2);

    const filteredRes = await request(app).get('/api/events?contract_id=ctr-list-001').set('Authorization', `Bearer ${masterToken()}`);
...
describe('POST /api/reviews/:id/override', () => {
  ...
  it('creates an override with 2+ authorizing entities', async () => {
    ...
    const res = await request(app)
      .post('/api/reviews/rev-ov-001/override')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
  ...
  it('returns 403 when fewer than 2 authorizing entities', async () => {
    ...
    const res = await request(app)
      .post('/api/reviews/rev-ov-002/override')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
  ...
  it('returns 404 for unknown review', async () => {
    const { app } = makeApp();
    const res = await request(app)
      .post('/api/reviews/no-such-review/override')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
  ...
  it('returns 400 when required fields are missing', async () => {
    const { app } = makeApp();
    const res = await request(app)
      .post('/api/reviews/rev-001/override')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
        authorizing_entities: ['a', 'b'],
      });
...
describe('POST /api/events — prohibited industry gate at intake', () => {
  it('rejects event creation when asset_reference contains a prohibited industry term', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-pi-001', contract_type: 'murabaha' });
    const res = await request(app)
      .post('/api/events')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
  ...
  it('allows event creation with a halal asset reference', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-pi-002', contract_type: 'murabaha' });
    const res = await request(app)
      .post('/api/events')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
...
describe('POST /api/instruments', () => {
  it('creates a waad instrument linked to a contract', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-w-001', contract_type: 'murabaha' });
  ...
  it('creates a zakat_obligation instrument', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-z-001', contract_type: 'musharaka' });
  ...
  it('returns 400 when instrument validation fails', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-kh-001', contract_type: 'murabaha' });
    const res = await request(app)
      .post('/api/instruments')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
...
describe('GET /api/instruments', () => {
  it('lists instruments for a contract', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-list-inst', contract_type: 'murabaha' });
    await request(app)
      .post('/api/instruments')
      .set('Authorization', `Bearer ${masterToken()}`)
      .send({
    ...
    const res = await request(app).get('/api/instruments?contract_id=ctr-list-inst').set('Authorization', `Bearer ${masterToken()}`);
    expect(res.status).toBe(200);
    expect(res.body).toHaveLength(1);
  ...
  it('returns 400 when contract_id is missing', async () => {
    const { app } = makeApp();
    const res = await request(app).get('/api/instruments').set('Authorization', `Bearer ${masterToken()}`);
    expect(res.status).toBe(400);
  });
});
...
describe('GET /api/instruments/:id', () => {
  it('returns instrument by id', async () => {
    const { app } = makeApp();
    await request(app).post('/api/contracts').set('Authorization', `Bearer ${masterToken()}`).send({ contract_id: 'ctr-gi-001', contract_type: 'murabaha' });
    ...
    const res = await request(app).get(`/api/instruments/${instrumentId}`).set('Authorization', `Bearer ${masterToken()}`);
    expect(res.status).toBe(200);
  ...
  it('returns 404 for unknown instrument', async () => {
    const { app } = makeApp();
    const res = await request(app).get('/api/instruments/no-such-id').set('Authorization', `Bearer ${masterToken()}`);
    expect(res.status).toBe(404);
  });
});
...
describe('GET /api/auth/me', () => {
  ...
  it('returns 404 when user does not exist in DB but token is valid', async () => {
    const { app } = makeApp();
    // Token references a user not in the DB
    const token = makeToken(OrgRole.compliance_officer, 'nonexistent-user');
